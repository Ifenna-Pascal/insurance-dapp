...
const ContractProvider = ({ children }) => {
  ...
  const viewCount = async () => {
    try {
      const contract = await getEthereumContract();
      // const count = ethers.utils.formatUnits(await contract.count(), "ether");
      // console.log(ethers.utils.formatUnits(count, "ether"), "count");
      // console.log(Math.pow(count, 18));
      console.log(await contract.count().toString());
    } catch (error) {
      console.log(error);
    }
  };
